...
function isFlexiProgram(program) {
    if (program.programCategory === "flexi") return true;
    const text = `${program.programTitle || ""} ${program.subTitle || ""}`.toLowerCase();
    return text.includes("flex") || text.includes("pgxpm") || text.includes("executive leadership");
}
    ...
    React.useEffect(() => {
        const fetchPrograms = async () => {
            try {
                ...
                setPrograms((Array.isArray(data) ? data : []).filter(isFlexiProgram));
            } catch (error) {
                console.error("Error fetching flexi programs:", error);
            } finally {
                setLoading(false);
            }
        };
                    ...
                    <AccordionContent className="bg-gray-50 p-4 space-y-3 dark:bg-black">
                        ...
                        {!loading && programs.length === 0 && (
                            <div className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground">
                                No Flexi programs are available in the database yet.
                            </div>
                        )}
                        {programs.map((program) => {
                            const link = `/program/flexi/${slugify(program.programTitle)}/${program._id}`;
                            return (
